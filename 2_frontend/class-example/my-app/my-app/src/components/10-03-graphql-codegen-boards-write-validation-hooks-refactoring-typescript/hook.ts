...
export const useBoardsWrite = () => {
  ...
  const onClickSubmit = async () => {
    ...
    alert("등록이 완료되었습니다.");
    router.push(
      `/section10/10-03-graphql-codegen-boards-validation-hooks-refactoring-typescript/${result.data.createBoard.number}`
    );
  };
  ...
  const onClickUpdate = async () => {
    ...
    alert("수정이 완료되었습니다.");
    router.push(
      `/section10/10-03-graphql-codegen-boards-validation-hooks-refactoring-typescript/${result.data.updateBoard.number}`
    );
  };
