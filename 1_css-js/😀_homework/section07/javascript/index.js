...
const JS_일기검색기능 = (event) => {
  ...
  timer = setTimeout(() => {
    ...
    const searchRes = 일기목록.filter((el) => el.제목.includes(searchInput));
    console.log(searchRes);
    const HTML_새로운일기도화지 = searchRes
      .map(
        (el, index) => `
      <a href="./detail.html?number=${index}">
        <div class="CSS_일기">
